...
void Foam::hePhaseChangeSolidThermo<BasicSolidThermo, MixtureType>::calculate()
{
...
/**/        if (this->updateT())
        ...
        fLCells[celli] = mixture_.ffa(TCells[celli]);//this->T_[celli];//1;mixture_.Tphf
//        Info << TCells[celli] << "\t" << mixture_.Tphf << endl;
    ...
    volScalarField::Boundary& fLBf = this->fL_.boundaryFieldRef();
    ...
    forAll(this->T_.boundaryField(), patchi)
    {
        ...
        fvPatchScalarField& pfL = fLBf[patchi];
        ...
        if (pT.fixesValue())
        {
            forAll(pT, facei)
            {
                ...
                pfL[facei] = mixture_.ffa(pT[facei]);
            }
        }
        else
        {
...
template<class BasicSolidThermo, class MixtureType>
Foam::hePhaseChangeSolidThermo<BasicSolidThermo, MixtureType>::
...
:
    ...
    (
        IOobject
        (
            Foam::basicThermo::phasePropertyName("f"),
            mesh.time().timeName(),
            mesh,
            IOobject::NO_READ,
            IOobject::AUTO_WRITE
...
template<class BasicSolidThermo, class MixtureType>
Foam::hePhaseChangeSolidThermo<BasicSolidThermo, MixtureType>::~hePhaseChangeSolidThermo()
{}
...
template<class BasicSolidThermo, class MixtureType>
void Foam::hePhaseChangeSolidThermo<BasicSolidThermo, MixtureType>::correct()
{
    if (debug)
    {
        InfoInFunction << endl;
...
Foam::hePhaseChangeSolidThermo<BasicSolidThermo, MixtureType>::Kappa() const
{
...
Foam::hePhaseChangeSolidThermo<BasicSolidThermo, MixtureType>::Kappa
(
    const label patchi
) const
{
